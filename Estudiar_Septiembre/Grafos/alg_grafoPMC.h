...
// de un grafo no dirigido ponderado y conexo G.
{
   ...
   vector<arista> aristas; //En vez de usar APO usamos un vector que ordenaremos desc.
   // Copiar aristas del grafo G en el vector aristas.
   for (vertice u = 0; u <= n-2; u++)
      for (vertice v = u+1; v <= n-1; v++)
         if (G[u][v] != INFINITO)
            aristas.push(arista(u, v, G[u][v])); //Meto en vector
   sort(aristas.begin(), aristas.end(), greater<arista>()); //Ordeno de mayor a menor
   for (size_t i = 0; i < aristas.size(); ++i)
   {
      vertice u = aristas[i].orig,
      v = aristas[i].dest; //Saco vértices del camino ponderado
      if (p.encontrar(u) != p.encontrar(v))
      { //Si son de != subconjuntos
         g[u][v] = g[v][u] = aristas[i].coste;
         p.unir(u, v);
      }
   }
   return g;

...
int nietosRec(Abin<T>& A, Abin<T>::nodo n)
{
    ...
    else
    {
        ...
        {
            if((A.hijoIzqdo(ni) != Abin<T>::NODO_NULO && A.hijoDrcho(ni) != Abin<T>::NODO_NULO && A.hijoIzqdo(nd) != Abin<T>::NODO_NULO) || (A.hijoIzqdo(ni) != Abin<T>::NODO_NULO && A.hijoDrcho(ni) != Abin<T>::NODO_NULO && A.hijoDrcho(nd) != Abin<T>::NODO_NULO) || (A.hijoDrcho(nd) != Abin<T>::NODO_NULO && A.hijoIzqdo(nd) != Abin<T>::NODO_NULO && A.hijoIzqdo(ni) != Abin<T>::NODO_NULO) || (A.hijoDrcho(nd) != Abin<T>::NODO_NULO && A.hijoIzqdo(nd) != Abin<T>::NODO_NULO && A.hijoDrcho(ni) != Abin<T>::NODO_NULO))
            {

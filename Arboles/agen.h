...
/* Métodos públicos*/
template <typename T> inline Agen<T>::Agen() : r{NODO_NULO}, numNodos{0} {}
...
template <typename T>inline void Agen<T>::insertarHijoIzqdo(nodo n, const T& e)
{
    assert(n != NODO_NULO);
...
template <typename T> inline void Agen<T>::insertarHermDrcho(nodo n, const T& e)
{
    assert(n != NODO_NULO);
...
template <typename T> inline void Agen<T>::eliminarHijoIzqdo(nodo n)
{
    ...
    assert(hizqdo->hizq == NODO_NULO); // y es hoja. El hermano drcho. pasa a ser el nuevo hijo izqdo.
    n->hizq = hizqdo->heder;
    delete hizqdo;--numNodos;
}
...
template <typename T> inline void Agen<T>::eliminarHermDrcho(nodo n)
{
    ...
    assert(hedrcho->hizq == NODO_NULO); // y es hoja. El hermano del hermano se convierte en el nuevo hermano drcho. de n.
    n->heder = hedrcho->heder;
    delete hedrcho;--numNodos;
}
...
template <typename T> inline bool Agen<T>::vacio() const {return numNodos == 0;}

template <typename T> inline size_t Agen<T>::tama() const {return numNodos;}
...
template <typename T> inline typename Agen<T>::nodo Agen<T>::padre(nodo n) const
{
    assert(n != NODO_NULO);
    return n->padre;
}

template <typename T> inline typename Agen<T>::nodo Agen<T>::hijoIzqdo(nodo n) const
{
    assert(n != NODO_NULO);
    return n->hizq;
}

template <typename T> inline typename Agen<T>::nodo Agen<T>::hermDrcho(nodo n) const
{
    assert(n != NODO_NULO);
    return n->heder;
}

template <typename T> inline Agen<T>::Agen(const Agen& A) : Agen{}
{
    r = copiar(A.r);   // Copiar raíz y descendientes.
    numNodos = A.numNodos;
}

template <typename T> inline Agen<T>& Agen<T>::operator =(const Agen& A)
{
    Agen B{A};
    std::swap(r, B.r);
...
template <typename T> inline Agen<T>::~Agen() {destruir(r); /*Vaciar el árbol.*/}
/* Métodos privados*/
// Destruye un nodo y todos sus descendientes
template <typename T> void Agen<T>::destruir(nodo& n)
{
    nodo hedrcho;
    if (n != NODO_NULO)
    {
...
template <typename T> typename Agen<T>::nodo Agen<T>::copiar(nodo n)
{
    ...
    if (n != NODO_NULO)
    {
        ...
        m = A.r; A.r = NODO_NULO; // Evita destruir la copia.
    }
    return m;
}
#endif // AGEN_H
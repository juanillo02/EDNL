...
particion crearparticion(const Grafo<bool> &M, size_t &islas)
{
    size_t n = M.numVert();
    Particion p(n);
    islas = n;
...
int buscarciudad(coordenada c, std::vector<coordenada> ciudad)
{
...
{
    size_t n = MA.numVert(), islas;
    GrafoP<tCoste> G(n);
    particion p = crearparticion(MA, islas);
    size_t m = (islas*(islas-1))/2;
    Apo<LineaAerea> LinAe(m);
    ...
    std::vector<LineaAerea> LA(m);
    while(!LinAe.vacia())
    {
        LineaAerea aux = LinAe.cima();
        LinAe.suprimir();
        int v = buscarciudad(aux.c1, ciudad), w = buscarciudad(aux.c2, ciudad);
        int isla1 = p.encontrar(v), isla2 = p.encontrar(w);
        if(isla1 != isla2) {
            LA.push_back(aux);
            p.unir(isla1, isla2); // unimos las islas
        }
    }
    return LA;
}

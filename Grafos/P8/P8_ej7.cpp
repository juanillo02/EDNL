...
//Cambiar ciudad por coordenadas
struct ciudad
{
    int x, y;
};
...
double Grecoland(std::vector<ciudad>& ciudadF, std::vector<ciudad>& ciudadD, std::vector<vertice>& costerasF, std::vector<vertice>& costerasD, vertice origen, vertice destino)
{
    ...
    puente p = Puenteminimo(ciudadF, ciudadD, costerasF, costerasD);
    p.v2 += nf;
    Grecoland[p.v1][p.v2] = p.coste;
    Grecoland[p.v2][p.v1] = p.coste;
    std::vector<vertice> VV(Grecoland.numVert());
    std::vector<tCoste> VD = Dijkstra(Grecoland, origen, VV);
    return VD[destino];
}